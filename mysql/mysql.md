...
大意为，查看某商品好评的第5001页。评论表的核心索引为index_product_id，因此对上述语句会想通过所有筛选出表中指定商品的评论，之后再按照is_good_comment筛选出评论中的好评。但由于is_good_comment没有索引，因此只能进行回表查询。

即对商品的每一条评论，都进行一次回表操作，筛选符合条件的数据，接着对筛选完毕的数据基于id进行倒序排序，此时又要基于临时磁盘文件进行排序操作，非常耗时间。排序完毕后，再基于limit 100000，20获取指定页数的数据，最后返回。

整个查询过程，会有几十万次的回表查询，以及十多万条数据的磁盘文件排序。非常耗时间
...
该sql通常会先执行括号里的子查询，子查询反而会使用PRIMARY聚簇索引，**按照聚簇索引的id值的倒序方向进行扫描**，扫描过程中会把符合WHERE product_id ='xx' and is_good_comment='1' 条件的数据筛选出来，然后再进行一次20条数据的回表即可



### 删除大量数据导致的慢sql

在个别情况下，慢sql查询并不是sql语句的问题，而是服务器负载太高，导致sql语句执行缓慢。如果感觉sql本身问题不大，可以尝试**排查当时MySql服务器的负载，尤其是磁盘，网络以及CPU负载**是否正常

**profileing工具**，这个工具可以对sql语句的执行耗时进行非常深入和细致的分析。使用工具过程需要在MySql界面执行

~~~sql
SET profiling = 1; //使用后会记录每个查询语句的query id

show profile cpu，block io for query  query id; //查看指定query id的执行时的cpu，block，io负载。其他语句可百度

show engine innodb status; // 查看innodb引擎的一些状态

~~~

使用 show engine innodb status后发现 history list length指标很高。当该指标很高时，意味着当前存在大量的undo多版本快照链条没被清理，这意味着当前可能有事务在长时间运行，导致多版本快照不能被purge清理。

经过排查发现，当时有一个长事务在删除大量数据，因为ReadView机制导致其他的查询语句进行查询时扫描数据扫到符合要求的数据时发现，该数据被删除了，只得继续扫描数据。

针对这个问题，要知道的一点是，永远不要在业务高峰期去运行那种删除大量数据的语句，因为这可能导致一些正常的SQL都变慢查询，因为那些SQL也许会不断扫描标记为删除的大量数据，好不容易扫描到一批数据，结果发现是标记为删除的，于是继续扫描下去，导致了慢查询！
...
MySql在执行增删改时，会记录binlog日志。

![image-20210422215109855](img/image-20210422215109855.png)

从库上有一个IO线程，该线程会负责和主库建立TCP连接，请求主库传输binlog日志给自己，主库上有IO dump线程，会负责通过TCP连接把binlog日志传输给从库的IO线程。

接着从库的IO线程会把读取到的binlog日志数据写入到自己本地的relay日志中去，然后从库上另外一个SQL线程会读取relay日志中的内容，进行日志重做，把所有在在主库上执行过的增删改操作，在从库上再做一遍，达到还原数据的过程。

简单来说，你只要给主节点挂上一个从节点，从节点的IO线程就会跟主节点建立网络连接，然后请求主节点传输binlog日志，主节点的IO dump线程就负责传输binlog日志给从节点，从节点收到日志后就可以回放增删改操作恢复数据。

### 数据延迟问题

可使用MySql运维工具 **percona-toolkit** 测试主从的延迟时间。

MySql5.7后就支持并行复制，可以在从库中设置 slave_parallel_workers>0，再将slave_parallel_type设置为LOGICAL_CLOCK就可开启并行复制。

如果要求刚写入的数据需要强制读到，可以通过中间件sharding-jdbc强制走主库



## 数据库高可用(MHA)

**高可用指所在主库宕机之后，就立马把从库切换为主库，然后所有请求都对从库切换为的主库去写入和读取**

**MHA工具**：Master High Availability Manager and Tools for MySQL。此工具可以监控主库的状态，感受到问题，可以把从库切换为主库

MHA也需要单独部署，分为两个节点，Manager和Node节点。Manager节点一般是单独部署一台机器，Node节点一般是部署在每台MySQL机器上的，因为Node节点需要通过解析各个MySQL的日志来进行操作。

Manager节点会通过探测集群里的Node节点去判断各个Node所在机器上的MySQL运行是否正常，如果发现某个Master故障了，直接把他的slave提升为Master，之后让其他的Slave都挂到新的Master上去。



## 分库分表

可以针对表的id进行hash，是数据均匀分布在不同的表上。同时使用中间件时设定好路由规则

分库分表的玩法基本都是这套思路，按业务id分库分表，建立索引映射表同时进行分库分表，数据同步到ES做复杂搜索，基本这套玩法就可以保证你的分库分表场景下，各种业务功能都可以支撑了。

必须要跨库和表查询和分页的时候，建议，第一，考虑一下是不是可以把你查询里按照某个主要的业务id进行分库分表建立一个索引映射表，第二是不是可以可以把这个查询里要的条件都放到索引映射表里去，第三，是不是可以通过ES来搞定这个需求。



分库分表后的再次扩容，建议一开始拆分时就根据数据的增量，多份一下，避免后续的麻烦。实在要扩，考虑好数据的迁移，业务代码的改动以及路由规则的修改